...
1. [테스트 전략: 무엇을 어떻게 테스트할 것인가](#테스트-전략-무엇을-어떻게-테스트할-것인가)
2. [next/navigation 모킹](#nextnavigation-모킹) — 글로벌 setup은 vitest-setup SSOT, 개별 오버라이드 패턴
3. [Server Component 테스트](#server-component-테스트)
4. [Server Actions 테스트](#server-actions-테스트)
5. [Middleware 테스트](#middleware-테스트)
6. [next/image, next/link 처리](#nextimage-nextlink-처리)
7. [Route Handler 테스트](#route-handler-테스트)
8. [Playwright E2E 패턴](#playwright-e2e-패턴)
...
### next/navigation 전역 모킹

`vi.mock('next/navigation')` 전역 setup은 `testing-vitest-setup.md` §vitest.setup.ts 참조. 개별 테스트에서 `vi.mocked(useRouter)`로 오버라이드한다.
